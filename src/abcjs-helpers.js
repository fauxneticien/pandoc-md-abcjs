...
function startTimer(visualObj, cursorControl) {
    cursorControl.onStart();
    const timingCallbacks = new ABCJS.TimingCallbacks(visualObj[0], {
        eventCallback: ev => ev ? cursorControl.onEvent(ev) : cursorControl.onFinished()
    });
    timingCallbacks.start();
}

// Render all <pre><code class="abcjs">...</code></pre> blocks
function renderAllAbcjsBlocks() {
    document.querySelectorAll("pre code.abcjs").forEach((codeBlock, idx) => {
        const abc = codeBlock.textContent.trim();
...
document.addEventListener("DOMContentLoaded", renderAllAbcjsBlocks);
